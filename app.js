...
async function fillFields(){
    ...
    document.getElementById("level").innerHTML = gameJson.level + " mərhələsi!";
    ...
    <br>
    <input type="checkbox" name="${catStringTeam1[0].split("_")[1]}" class="input_checkbox1" onchange='calc()'> ${catStringTeam1[0].split("_")[2]} (${catStringTeam1[0].split("_")[1]}) <button onclick='show_question("${catStringTeam1[0].split("_")[0]}")'>Sual</button><br><br>
    <input type="checkbox" name="${catStringTeam1[1].split("_")[1]}" class="input_checkbox1" onchange='calc()'> ${catStringTeam1[1].split("_")[2]} (${catStringTeam1[1].split("_")[1]}) <button onclick='show_question("${catStringTeam1[1].split("_")[0]}")'>Sual</button> <br><br>
    <input type="checkbox" name="${catStringTeam1[2].split("_")[1]}" class="input_checkbox1" onchange='calc()'> ${catStringTeam1[2].split("_")[2]} (${catStringTeam1[2].split("_")[1]}) <button onclick='show_question("${catStringTeam1[2].split("_")[0]}")'>Sual</button> <br><br>`;
    document.getElementById("categoties_team1").innerHTML = categories_team1;
    document.getElementById("points_team1").innerHTML = `<span style="font-size: 25px; font-weight: 800;">Potensial bal: ${parseInt(catStringTeam1[0].split("_")[1], 10) + parseInt(catStringTeam1[1].split("_")[1], 10) + parseInt(catStringTeam1[2].split("_")[1], 10)}</span>
    <br><br> <span style="color: green; font-size: 25px; font-weight: 800;">Toplanılmış bal: <span style="color: green;" id="collected1">0</span></span>`;

    let categories_team2 = `
    <br>
    <input type="checkbox" name="${catStringTeam2[0].split("_")[1]}" class="input_checkbox2" onchange='calc()'> ${catStringTeam2[0].split("_")[2]} (${catStringTeam2[0].split("_")[1]}) <button onclick='show_question("${catStringTeam2[0].split("_")[0]}")'>Sual</button><br><br>
    <input type="checkbox" name="${catStringTeam2[1].split("_")[1]}" class="input_checkbox2" onchange='calc()'> ${catStringTeam2[1].split("_")[2]} (${catStringTeam2[1].split("_")[1]}) <button onclick='show_question("${catStringTeam2[1].split("_")[0]}")'>Sual</button> <br><br>
    <input type="checkbox" name="${catStringTeam2[2].split("_")[1]}" class="input_checkbox2" onchange='calc()'> ${catStringTeam2[2].split("_")[2]} (${catStringTeam2[2].split("_")[1]}) <button onclick='show_question("${catStringTeam2[2].split("_")[0]}")'>Sual</button> <br><br>`;
    document.getElementById("categoties_team2").innerHTML = categories_team2;
    document.getElementById("points_team2").innerHTML = `<span style="font-size: 25px; font-weight: 800;">Potensial bal: ${parseInt(catStringTeam2[0].split("_")[1], 10) + parseInt(catStringTeam2[1].split("_")[1], 10) + parseInt(catStringTeam2[2].split("_")[1], 10)}</span>
    <br><br> <span style="color: green; font-size: 25px; font-weight: 800;">Toplanılmış bal: <span style="color: green;" id="collected2">0</span></span>`;
}

function calc(){
    let point1 = 0;
    let point2 = 0;
    let team1checkboxes = document.getElementsByClassName("input_checkbox1");
    for (let i = 0; i < team1checkboxes.length; i++) {
        if(team1checkboxes[i].checked){
            point1 += parseInt(team1checkboxes[i].name, 10);
        }           
    }

    let team2checkboxes = document.getElementsByClassName("input_checkbox2");
    for (let i = 0; i < team1checkboxes.length; i++) {
        if(team2checkboxes[i].checked){
            point2 += parseInt(team2checkboxes[i].name, 10);
        }           
    }
...
function show_question(currency){
    create_square(currency);
    document.getElementById("question-area").innerHTML = "";
    document.getElementById("question-area").style = "border: none";
}

async function create_square(currency){
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        for (let i = 1; i <= 4; i++) {
            if(contain(JSON.stringify(numJson), counter+1))  {
                string += `<td id="td_${counter+1}" style="cursor: pointer; border: 1px solid black; font-size: 30px; height: 85px; width: 200px; text-align: center; background-color: lightgreen;" 
                onclick='use(${counter+1}, "${currency}", "td_${counter+1}")'>${counter+1}</td>`; 
            }
            else{
                string += `<td id="td_${counter+1}" style="cursor: default; border: 1px solid black; font-size: 30px; height: 85px; width: 200px; text-align: center; background-color: darkred;">${counter+1}</td>`; 
            }      
            counter++;
        }
        string += `</tr>`;
    ...
    string += `</tbody></table>`;
    document.getElementById("square").style = "margin-left: 20%;";
...
async function use(i, c, td_id){
    ...
    string += `<br><button id="start_timer" style="margin: auto 45.5%;" name="button" onclick="timer()">Cavablar</button><br> `;
...
function finish(){
    ...
    if(point1 == point2){
        let team1 = document.getElementById("team1").innerHTML;
        let team2 = document.getElementById("team2").innerHTML;
        window.location.href = 'show_questions/show_question.html?game_id=' + game_id + '&point1=' + point1 + '&point2=' + point2;
        return;
    }
